...
void sueldoMayorMenor(){
	...
	while ((data=fgetc(ARCHmayorMenor))!=EOF){
		...
		if(empleados.sueldo > sueldoMayor){
			...
			strcat(nombreSueldoMayor," ");
			strcat(nombreSueldoMayor,empleados.apellido);
		}
		...
		if(empleados.sueldo < sueldoMenor){
			...
			strcat(nombreSueldoMenor," ");
			strcat(nombreSueldoMenor,empleados.apellido);
		}
	}
